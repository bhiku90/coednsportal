...
const showData = async()=> {
    apiData = await axios.get("https://coednssecurity.in/healthmediator/top25canada", {   
    }).then((response) => {
        //console.log(response.data);
        if (response.status == 200 && Object.keys(response.data).length !== 0) {
          return response.data.healthindexcanada;
        }
        else {
          return {};
        }    
      ...
      let sortedData1 = Array.from(uniqueNameservers.values()).slice(0,25-nicuniName.size).concat(nicdata);
  ...
  return (
    <div className="textcontainer">
            <div className="section-title text-center">
                 <h2 style={{color:"#fff"}}>Health of Top Nameservers accessed from Canada</h2>
