...
export const Malicious = ({ setDataInMalTable }) => {
  ...
  const fetchDetails = async (e) => {
    ...
    malData = await axios
      .post("https://coednssecurity.in/healthmediator/malicious", {
        domain: `${inputDomain}`,
      })
      .then((response) => {
        if (response.data.status == "fail") {
          ...
          //console.log("NOT ACTIVE...."+response.data);
          setnamestatus("No Such Domain Exists");
          setTimeout(() => {
            removeContent();
          }, 2000);
    ...
    let malDataRespStatus = malData.status || [];
    let malDataResp = malData.domain || [];
    let nameServerListLength = malDataRespStatus.length || 0;
      ...
      <div className="col-xs-12 col-sm-12">
        <div id="mainrender" className="search box container">
          ...
          <Box sx={{ typography: "body1", color: "GrayText", fontSize: 15 }}>
            <b>Note : </b>The results are based on an AI-based Binary Classifier
            Model. We solicit your contribution to strengthen the classifier
            model.
            <br />
            Please contribute to our <b>SPAM BOX</b> and help us in improving
            the intelligence of our tools.
          </Box>
        </div>
      </div>
